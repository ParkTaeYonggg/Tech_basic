...
- 서버사이드 어플리케이션
  html에 들어간 요소들의 해석을 서버에서 해결하고 화면에 보여줌.
  -> 미리 완성되어서 오기 때문에 검색 시 (seo) 상위 노출 된다는 장점이 있음.
  -> 캐싱의 장점도 가지고 있음.
  
  단점으로는 html 요소 해석을 서버에서 하기 때문에 무언가 변화가 있으면 재해석 하는데 시간이 csr보다 길게 걸림.
 
- 클라이언트사이드 어플리케이션
  해석된 코드를 받는게 아닌 json 혹은 xml형태로 전당받아서 js를 통해 분석 후 화면에 뿌려줌.
  -> 화면에서 해석을 하고 보여주기 때문에 필요한 부분만 짤라서 보여줄 수도 있음. 그래서 속도가 좀 빠름. -> spa 방식과 어울렸었음.
  
  단점으로는 처음 json 혹은 xml같은 것들을 해석할 js들이 미리 만들어져야 하기 때문에 제일 처음 랜더 시 오래 걸릴 수 있음.
  -> 그리고 클라이언트단에 와서 해석을 하기 때문에 미리 해석된 형태로 오는 서버사이드에 비해 seo에 걸리는데 우선순위가 뒤로 밀릴 수 있음.
  
## 컬렉션 프레임워크 인터페이스에서 {}과 ()의 차이
-> 중괄호는 하나의 함수와 같은 역할이다. 즉, 표현식을 사용하여 하나의 리턴 값을 받게 된다.
-> ()가 있거나 아예 없는 형태로 작성하게 된다면 표현식이 사용이 불가능 하지만 다수의 리턴 값을 받을 수 있다.

## 동기, 비동기 스테이트 관리
-> 동기와 비동기 식으로 스테이트를 관리할 수 있다.
(ex) 동기 : const [zz, setZZ] = useState();
(ex) 비동기 : const [zz, setZZ] = () => useState();

## useContext
useContext는 리액트 자체에서 제공하는 스테이트 상속 장치다.
가장 상위트리(조상 -> 부모 -> 자식 -> 자식의 자식일 때 조상에서 자식의 자식에게 전달하고자 할 때 등등)에서 전달하고자 하는 컴포넌트에 데이터를 전달할 수 있는 것이다.
-> 컨텍스트를 사용하는 컴포넌트 전체에서 리랜더를 일으키고 싶지 않으면 유즈메모 등으로 캐싱해두어 사용하자. --> 유의할 점이다.

![스크린샷 2022-03-05 오후 11 11 07](https://user-images.githubusercontent.com/86910922/156887272-8a666b09-41c4-46be-b9bc-cf462411eb42.png)
![스크린샷 2022-03-05 오후 11 10 40](https://user-images.githubusercontent.com/86910922/156887275-82572161-a873-4963-90fc-164c2b120bd8.png)

사용처는 다크모드, 화이트모드?, 언어 등등 다양한 곳에 사용할 수 있다.
